...
const[items, setItems]= React.useState([])
    ...
    const handleSearch = event =>{
        ...
        searchPokemon(query,results=> setItems(results) )
    }
    return<div>
        <form className='form-class flex flex-col items-center justify-center mt-20' onSubmit={handleSearch}>
             ...
             <button className="button border-2 border-[#5b21b6] bg-[#a78bfa] p-3 rounded-lg" type="submit">ability</button>
         </form>
         <ul>{items.map(item =>
        <li>Name:{item.ability.name} <a href=""> Url:{item.ability.url}</a></li>)}
         </ul>
    </div>
}
// const [items, setItems] = React.useState([])
