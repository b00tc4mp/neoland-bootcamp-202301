...
function Item({ element, onUpdateVisibility, onDelete, onToggleLike, onChangeColor }) {
    ...
    return <li className={`${bgColor}  w-[30ch] p-3 rounded-lg border-solid border-2 border-[#6b7280]`} key={element._id}>
        <div className="flex flex-row justify-end">
            ...
            {element.user === sessionStorage.userId &&
                ...
                <button className="border-solid border-2 border-[#6b7280] w-6 h-6 text-center m-1" data-id={element._id} onClick={handleUpdateVisibility} data-visibility={element.visibility}>🌍</button>
                :
                <button className="border-solid border-2 border-[#6b7280] w-6 h-6 text-center m-1" data-id={element._id} onClick={handleUpdateVisibility} data-visibility={element.visibility}>🛑</button>)
            }
