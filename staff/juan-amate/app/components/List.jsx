function List() {
    ...
    return <ul className="flex flex-col items-center">
        {stickies.map(sticky => <li key={sticky.id} className='border rounded-md bg-white p-3 m-3 w-[40ch] text-right'>
            <div className="">
                {sticky.user === sessionStorage.email && <button className="bg-blue-600 border border-gray-400 m-0.5 rounded-md h-8 w-8" id={sticky.id} data-visibility={sticky.visibility} onClick={handleUpdateVisibility}>🚦</button>}

                {sticky.user === sessionStorage.email && <button className="bg-blue-600 border border-gray-400 m-0.5 rounded-md h-8 w-8" id={sticky.id} onClick={handleDelete}>❌</button>}
            ...
            <p className="text-xl pt-5 text-left" id={sticky.id} contentEditable={sticky.user === sessionStorage.email} onKeyUp={handleUpdateText} suppressContentEditableWarning={true}>{sticky.text}</p>
