    ...
    return <li className={`${bgColor} w-[60ch] p-10 border-double  border-2`} key={element._id}>
        <div className="text-right">
            {element.user === sessionStorage.userId && <>
                ...
                <button className="w-5 h-5  text-[#020200] m-1" data-id={element._id} data-visibility={element.visibility} onClick={handleUpdateVisibility}>{element.visibility === 'public' ? '-' : '+'}</button>

            </>}
            {element.user === sessionStorage.userId && <button className="w-5 h-5  text-[#030301] m-1" data-id={element._id} onClick={handleDelete}>x</button>}
        </div>
