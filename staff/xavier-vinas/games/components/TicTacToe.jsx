function TicTacToe() {
    ...
    // logica con ternario  para cambiar simbolo    
    const switchSymbol = () => {
        // if (symbol === 'X')
        //     setSymbol('O')
        // else
        //     setSymbol('X')
        setSymbol(symbol === "X" ? "O" : "X") // si el valor actual de symbol es "X" ponle "O" y si no pon "X"
    }

    const handleB1 = () => {
        if (!b1) {
            setB1(symbol)
            switchSymbol()// si el valor actual de symbol es "X" ponle "O" y si no pon "X"
    ...
    // logica de combinaciones ganadoras 
